...
public final class GuildService {
    ...
    public static @NotNull CompletableFuture<@NotNull Void> update(
            ...
            @NotNull GuildModel updatedGuild
    ) {
        return PlayerRepository.get(PlayerRepositoryOptionAttribute.UUID, performedBy.getUniqueId())
                .thenCompose(player -> {
                    if (player == null) {
                        MineAsterisk.getPluginLogger().info("Unable to update Guild: Player doesn't exist.");
                    ...
                    if (playerGuild == null) {
                        MineAsterisk.getPluginLogger().info("Unable to update Guild: Player doesn't have a Guild.");
                        return CompletableFuture.completedFuture(null);
                    }

                    if (playerGuild.getOwner().getId() != player.getId()) {
                        MineAsterisk.getPluginLogger().info("Unable to update Guild: Player is not the Guild owner.");
                        return CompletableFuture.completedFuture(null);
                    }
                    ...
                    return GuildRepository
                            .get(GuildRepositoryOptionAttribute.ID, playerGuild.getId())
